...
3. インストール途中でインストーラー画面が消える場合には(その１)  
...
またパーティションの手動設定(Manual Partitioning)を行なう場合、手動設定を選択するよりも前に、いったん他OSとの「共存」または「パーティションの置換」などを選んでみてください。先に「共存」「置換」をいったん選ぶことによって、パーティション情報の収集を終了させてから、手動設定を選ぶことで、異常終了を回避しやすくなります。
```sh
