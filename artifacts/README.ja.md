...
またディストリビューションを超えて、アプリケーションを配布する仕組みとして、[Flatpak](https://flatpak.org/)と[Snaps](https://snapcraft.io/about)といった仕組みが作られてきました。Manjaro LinuxもFlatpakとSnapsにも対応していますので、他のLinuxディストリビューションと同じように、いろんなアプリケーションが動作可能です。(AppImage形式のソフトも`gearlever`パッケージなどをインストールすれば、扱いやすいです。)  
