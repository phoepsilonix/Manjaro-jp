...
cat Japanese-pkgs.txt Japanese-pkgs-root.txt iso-profiles-orig/manjaro/{gnome,kde,xfce,architect}/Packages-* iso-profiles-orig/shared/Packages-* |sed "s|KERNEL|$KERNEL|"|sed -e 's|#.*||' |sed -e 's|^>.* ||' -e '/\.\.\/\.\./d'|sort -u | xargs
cat Japanese-pkgs.txt Japanese-pkgs-root.txt iso-profiles-orig/manjaro/{gnome,kde,xfce,architect}/Packages-* iso-profiles-orig/shared/Packages-* |sed "s|KERNEL|$KERNEL|"|sed -e 's|#.*||' |sed -e 's|^>.* ||' -e '/\.\.\/\.\./d'|sort -u | xargs sudo pacman -Syyw --noconfirm systemd-libs polkit xorg-server-common
