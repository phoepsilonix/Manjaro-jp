...
cat Japanese-pkgs.txt Japanese-pkgs-root.txt iso-profiles-orig/manjaro/{gnome,kde,xfce,architect,shared}/Packages-* |sed 's|KERNEL|linux68|'|sed -e 's|#.*||' |sed -e 's|^>.* ||'| sort -u | xargs sudo pacman -Syyw --noconfirm lib32-nvidia-utils lib32-nvidia-390xx-utils lib32-nvidia-470xx-utils systemd-libs polkit xorg-server-common
